...
const Publisher = () => {
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      fetchPublishers(user.accessToken);
    }
  }, []);

  const fetchPublishers = async (authToken) => {
    try {
      const response = await fetch(
         get_url('root-path')+"/ocean_api/api/publishers",
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
         get_url('root-path')+"/ocean_api/api/publisher/add", 
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to add publisher');
      }
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
         get_url('root-path')+`/ocean_api/api/publisher/${currentEditItem.id}`, 
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  if (loading) {
    return <div className="text-center mt-4">Loading publishers...</div>;
  }
  ...
  return (
    <div className="container mt-4">
      ...
      {publishers.length > 0 ? (
        ...
        <div className="alert alert-info">No publishers available</div>
      )}
