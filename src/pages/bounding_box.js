...
const BoundingBox = () => {
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      fetchBoundingBoxes(user.accessToken);
    }
  }, []);

  const fetchBoundingBoxes = async (authToken) => {
    try {
      const response = await fetch(
        get_url('root-path')+"/ocean_api/api/bounding_box",
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        get_url('root-path')+"/ocean_api/api/bounding_box/add", 
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to add bounding box');
      }
      ...
      await fetchBoundingBoxes(token);
    } catch (err) {
      setError(err.message);
    }
  };

  const handleEditClick = (boundingBox) => {
    setCurrentEditItem(boundingBox);
    setFormData({
      name: boundingBox.name,
      west_bound_longitude: boundingBox.west_bound_longitude || '',
      east_bound_longitude: boundingBox.east_bound_longitude || '',
      south_bound_latitude: boundingBox.south_bound_latitude || '',
      north_bound_latitude: boundingBox.north_bound_latitude || '',
      crs: boundingBox.crs || ''
    });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        get_url('root-path')+`/ocean_api/api/bounding_box/${currentEditItem.id}`, 
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to update bounding box');
      }
  ...
  if (loading) {
    return <div className="text-center mt-4">Loading bounding boxes...</div>;
  }
  ...
  return (
    <div className="container mt-4">
      ...
      {boundingBoxes.length > 0 ? (
          ...
          <tbody>
            {boundingBoxes.map((box) => (
              <tr key={box.id}>
                <td>{box.id}</td>
                <td>{box.name}</td>
                <td>{box.west_bound_longitude || '-'}</td>
                <td>{box.east_bound_longitude || '-'}</td>
                <td>{box.south_bound_latitude || '-'}</td>
                <td>{box.north_bound_latitude || '-'}</td>
                <td>{box.crs || '-'}</td>
        ...
        <div className="alert alert-info">No bounding boxes available</div>
      )}

      {/* Add Modal */}
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Add New Bounding Box</Modal.Title>
        </Modal.Header>
          ...
          <Modal.Body>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                required
                placeholder="Bounding box name (e.g., Vaitupu)"
              />
            </Form.Group>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Edit Bounding Box</Modal.Title>
        </Modal.Header>
